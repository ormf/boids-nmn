...
(defun define-elisp-code ()
  (let ((swank::*emacs-connection* *emcs-conn*))
    (swank::eval-in-emacs
     `(progn
        (setq boids-preset-file ,(namestring (merge-pathnames "audio-tmp.lisp" (asdf:system-source-directory :boids-nmn))))
        (find-file boids-preset-file)
        (set-window-dedicated-p (get-buffer-window "audio-tmp.lisp" t) t)
        (set-window-dedicated-p (get-buffer-window "curr-preset.lisp" t) t)
        (load ,(namestring (merge-pathnames "edit-orgel-presets.el" (asdf:system-source-directory :boids-nmn))))
        ) t)))
...
(defparameter *ip-local* "192.168.67.13")
...
;;;  (setf *ip-local* "192.168.67.11")
  (setf *ip-local* "192.168.11.13")
  (setf *ip-tablets* '("192.168.11.31"))
...
(defun init-flock ()
  (setf *basedir*  (asdf:system-source-directory :boids-nmn))
  (cd *basedir*)
...
;;  (setf *audio-presets-file* (bs-full-path "presets/flock-2020-06-11-audio.lisp"))
  ...
  (setf *audio-presets-file* (bs-full-path "presets/boids-nmn-01-audio.lisp"))
;;;  (setf *bs-presets-file* (bs-full-path "presets/flock-solo-01-bs.lisp"))
  (setf *bs-presets-file* (bs-full-path "presets/boids-nmn-01-bs.lisp"))
  (init-cc-presets)
